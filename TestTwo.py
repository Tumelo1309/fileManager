...
for file in common_files:
    temp_device_file = device_path + "/" + file
    temp_usb_file = usb_path + "/" + file

    if os.path.getsize(temp_device_file) > os.path.getsize(temp_usb_file):
        os.replace(temp_device_file,usb_path)
        print("Replace successful from device to usb")
        ...
        os.replace(temp_usb_file,device_path)
        print("Replace successful from usb to device")